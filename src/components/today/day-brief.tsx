...
export function DayBrief({ userName, summary, overdue, today, thisWeek }: Props) {
  ...
  return (
    <div className="bg-white border border-black/5 rounded-card shadow-card p-4 md:p-5 mb-5">
      <div className="flex items-start gap-3">
        <div className="flex-shrink-0 inline-flex items-center justify-center w-9 h-9 rounded bg-light-steel/[0.18] border border-light-steel/40">
          <Sparkles className="w-4 h-4 text-graphite" />
        </div>
        <div className="flex-1 min-w-0">
          <h2 className="text-base font-semibold text-graphite leading-snug">
            {greeting}{firstName ? `, ${firstName}` : ''}
            {caughtUp && <span className="text-success ml-2">— all caught up</span>}
          </h2>
          <p className="text-xs text-steel mt-1 font-mono">{summaryLine}</p>
          {mostPressing && !caughtUp && (
            <p className="text-xs text-graphite mt-2 leading-relaxed">
              <span className="text-steel/70">Start with:</span>{' '}
              <span className="font-semibold">{mostPressing.title}</span>
              ...
              <span className="text-steel ml-1.5 font-mono text-[11px]">
